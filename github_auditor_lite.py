...
def get_all_repos(org):
    """Fetches all non-archived repositories for the organization."""
    repos = []
    page = 1
    while True:
...
def audit_repo(repo):
    ...
    rules = get_effective_branch_rules(GITHUB_ORG, repo['name'], repo['default_branch'])
...
def main():
    if not GITHUB_ORG or not GITHUB_TOKEN:
        print("Please set GITHUB_ORG and GITHUB_TOKEN environment variables.")
        return

    print(f"Starting SOC 2 Branch Protection Audit for Org: {GITHUB_ORG}")
    ...
    # Export Report using native CSV module instead of pandas
